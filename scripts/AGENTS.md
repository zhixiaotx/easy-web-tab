...
├── test-business-core.ts    # businessCore.ts 纯函数测试（24 断言 T1-T24：种子/归一化/内置支出补回/商品分类空数组尊重/分类 CRUD（add/rename/toggle/move/delete）/soldCount/calcDailyRevenue/calcDailyCost（纯 COGS）/calcDailyLossAmount/calcInventory/calcBusinessStats（纯 COGS 成本/expenseTotal 支出合计/负毛利率/缺失商品计 0/空数据）/lowStockProducts（停售过滤）/calcPurchaseTotals/calcBroughtOutTotals/排行/趋势（按收摊记录口径）/每日一柱堆叠分色坐标 businessTrendBars+compactAmount/filterPurchasesByCategory（all/分类过滤/已删商品排除/未分类排除/空分类）；npm run test:business）
├── test-habit-core.ts       # habitCore.ts 纯函数测试（npm run test:habits，node --experimental-strip-types）
├── test-pomodoro-core.ts    # pomodoroCore.ts 纯函数测试（npm run test:pomodoro）
├── test-spotlight-core.ts   # spotlightCore.ts 纯函数测试（searchAll/rankResults 6 类跨源搜索排序；npm run test:spotlight）
├── test-snapshot-core.ts    # snapshotCore.ts 纯函数测试（makeSnapshotId/normalizeSnapshotList/pushSnapshot/findSnapshot；npm run test:snapshot）
├── resolve-extensionless.mjs # Node 自定义 loader：解析无扩展名相对导入（`./countdownCore`）+ `@/` 别名 → 学生 core 测试运行必需（每个 test:student-* 都带 `--experimental-loader ./scripts/resolve-extensionless.mjs`；纯 node --experimental-strip-types 会 ERR_MODULE_NOT_FOUND/ERR_UNSUPPORTED_DIR_IMPORT）
├── test-student-menu-core.ts        # studentMenuCore 纯函数测试（npm run test:student-menu）
...
├── test-student-achievement-core.ts # studentAchievementCore（npm run test:student-achievement）
├── test-student-parenttask-core.ts  # studentParentTaskCore（npm run test:student-parenttask）— 全部 14 个 test:student-* 都需 `--experimental-loader ./scripts/resolve-extensionless.mjs`；`npm run test:student` 聚合
...
├── qa-business.mjs        # Playwright UI QA：销售记账全量契约 S1-S9（后台/复用 vite dev 16718-16726 → 注入 IndexedDB easy-web-tab v6 store 'business' → /business 断言：S1 左树 7 项+首页统计卡数值（含支出）+分类/商品排行（bizhome-cat-/bizhome-prod- 各 2 行）+无快捷操作/S2 商品页 tabs+新增+停售开关/S3 进货卡片网格（桌面 5 列）+分类 tabs（全部+5 可见分类）+ 卡片分类徽标（bizpur-cat-badge-qa-b1=小吃）+ 分类过滤（小吃 1 卡/饮品空态）+ 新增自动合计/S4 收摊卡片四项（营业额 200/成本 68/利润 132/损耗 5）+ 编辑弹框四项预览（50/20/30/0）+ 同日 upsert 四项重算/S5 支出 tabs+分类管理新增+新增支出/S6 库存卡片网格 3 卡 5 列（卡片含库存剩余：qa-p1=100/qa-p2=0/爆米花=0）+预警仅 1 张（停售爆米花不预警）+阈值 0 清空/S7 统计页无排行（已移首页）+趋势每日一柱堆叠分色按收摊记录口径（仅收摊日 2 段 成本+利润/单标签/宽>12/viewBox 320/网格线 5）/S8 设置弹窗销售记账 tab/S9 移动端 375 无横向滚动+明暗截图；证据存 .omo/evidence/business/；node scripts/qa-business.mjs）— 与其它 QA 脚本禁止并行（同端口域）
├── qa-business-ep.mjs     # Playwright UI QA：销售记账 Element Plus 换皮契约 E1-E8（后台/复用 vite dev 16718-16726 → 注入 IDB 'business' 更小数据（qa-a1/a-b1）→ /business 断言：E1 商品页 tabs+在售开关往返+详情抽屉 el-drawer+编辑/新增 el-dialog 全字段/E2 进货 tabs 6+徽标小吃+网格 5 列+弹框日期/选品/合计交互/E3 收摊卡四项 200/68/132/5+弹框 el-* 全套/E4 支出无 tabs+分类管理/新增+弹框选品/金额/备注/E5 库存阈值才 el-input+双预警+卡库存 75/0+溯源弹框 2 源+阈值 0 清空/E6 统计 6 个 el-radio-button+网格 5/双段柱/标签/折线/双环+模式切换 1→1→2/E7 设置弹窗 sales 子 tab biz-base+摊位/阈值 el-*+分类管理弹框 5 行/E8 商品分类管理弹框 el-dialog+行内 el-checkbox/input/button+新增玩具 5→6 行；每面板断言「零原生残留」（scanNativeResidue 扫描 input/select/textarea/dialog 换皮）+明暗图片存 .omo/evidence/business-ep/；脚本尾部聚合 verdict 需「E1-E8 全 PASS 且 console_errors=0」才 PASS（QA 抓 console 错误，DataCloneError/ElInputNumber 字符串 modelValue 会 flip 失败）；node scripts/qa-business-ep.mjs）— 与其它 QA 脚本禁止并行（同端口域）
...
| 销售记账 UI QA | `qa-business.mjs` | Playwright chromium headless：注入 IDB v6 store 'business'（2 商品/1 进货/1 收摊/2 支出/阈值 100）→ /business 断言 S1-S9（左树 7 项+统计卡数值+分类/商品排行/商品 tabs+新增+停售/进货卡片网格 5 列（bizpur-card-<id>）+分类 tabs（bizpur-cat-* 全部+5 可见分类）+ 卡片分类徽标（bizpur-cat-badge-<id>=小吃）+ 分类过滤（小吃 1 卡/饮品空态）+ 新增自动合计/收摊卡片四项（营业额 200.00/成本 68.00/利润 132.00/损耗 5.00）+ 编辑弹框四项预览（50.00/20.00/30.00/0.00）+ 同日 upsert 四项重算/支出 tabs+分类管理新增分类/库存卡片网格 3 卡 5 列（卡片含库存剩余：qa-p1=100/qa-p2=0/爆米花=0）+预警仅 1 张（停售爆米花不预警）+阈值 0 清空/统计页无排行（已移首页）+趋势每日一柱堆叠分色 SVG（仅收摊日 2 段 成本+利润/单标签/宽>12/viewBox 320/模式切换）/设置弹窗销售记账 tab/移动端 375 无横向滚动+明暗截图）；后台 vite dev 16718-16726 复用或自起（与其它 QA 脚本禁止并行）；证据存 `.omo/evidence/business/`；运行 `node scripts/qa-business.mjs` |
| 销售记账 EP 换皮 QA | `qa-business-ep.mjs` | Playwright chromium headless：注入 IDB v6 store 'business'（qa-a1/q-a1 + qa-b2 进货，qa-p1 库存 75 预警 / qa-p2 库存 0 阈值 0 双预警）→ /business 断言 E1-E8 契约（8 面板全部 Element Plus 化：el-radio-button/el-drawer/el-dialog/el-input/el-select/el-input-number/el-checkbox/el-button/el-pagination，data-testid 全保；每面板 scanNativeResidue 零原生残留；E8 分类管理新增玩具 5→6 行）；脚本尾部聚合 verdict 需「E1-E8 全 PASS 且 console_errors=0」才 PASS——QA 抓 console 错误，任何 `DataCloneError`/ElInputNumber 字符串 modelValue 警告都会 flip 失败；后台 vite dev 16718-16726 复用或自起（与其它 QA 脚本禁止并行）；证据存 `.omo/evidence/business-ep/`；运行 `node scripts/qa-business-ep.mjs` |
...
- `qa-diary.mjs` / `qa-ledger-charts.mjs` / `qa-workbench-home.mjs` / `qa-workbench-onescreen.mjs` / `qa-reminder-upgrade.mjs` / `qa-reminder-toggle.mjs` / `qa-business.mjs` / `qa-business-ep.mjs` / `qa-student-ui.mjs` / `qa-student-onescreen.mjs` / `qa-student-reading-grid.mjs` / `qa-student-ep-*.mjs`（7 个）共用 16718-16726 端口域 — 这些 QA 脚本禁止相互并行（同端口域抢占 dev server / 端口冲突）
- `store/workbenchBusiness.saveBusiness()` 对商品/支出两个分类数组做 `toRaw().map(c => ({...toRaw(c)}))` 元素级深拷贝：`businessCore` 分类 CRUD 把 reactive 数组传入后经 `[...list, next]` / `list.map(...)` 返回新数组，元素仍是 Proxy——整体 `toRaw` 只解外层、IDB structured clone 抛 `DataCloneError: #<Object> could not be cloned`（分类写路径唯一，E8 新增分类触发）；勿改回仅整体 `toRaw`
