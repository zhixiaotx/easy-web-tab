...
3 helper scripts + 5 test scripts + 1 Playwright UI QA script: build-time icon generation, production HTTP server, one-time utility, pure-function test runners, chart UI QA (injects IndexedDB data before asserting). CommonJS + PowerShell + TS (package is ESM — `.cjs` extension required; test scripts run via `node --experimental-strip-types`).
