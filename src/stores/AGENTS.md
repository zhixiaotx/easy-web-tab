...
├── settings.ts           # Dialog size settings contract + 工作台菜单状态/变更（顺序/名称/显示开关）+ 导航筛选栏展开态 (useAppSettingsStore, DIALOG_DEFAULTS / DIALOG_LABELS)
...
| Dialog size settings | `settings.ts` | `useAppSettingsStore` — per-dialog width/height contract, clamp 400-1600px / 30-100vh；另含 `navFiltersExpanded`（导航筛选栏展开态，默认 false 收起） |
