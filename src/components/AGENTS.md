...
└── workbench/            # 个人工作台 12 SFC: 10 面板 + 健康管理 tabs 容器 + 定时提醒只读区块 (data persisted to IndexedDB via `useIdb.ts`)
    ...
    ├── WorkbenchNotes.vue       # 便签面板（类型切换 普通/时光轴 tabs + 搜索 + 分类筛选 chips + 分类管理弹窗 + 时光轴卡片竖排时间轴/快速追加/条目内联编辑删除，data-testid 前缀 nt-）
...
| Countdown management | `CountdownManager.vue` + `workbench/WorkbenchCountdown.vue` | 管理端/工作台表单：6 种重复规则选择器（weekly 周几多选 + 工作日快捷钮 / monthly 几号 / interval 分钟）+ 分类 6 选（work/life/study/exercise/diet/sleep）+ 校验；`repeatLabel`/`categoryLabel` 徽标 |
| Countdown display | `CountdownModal.vue` | 前台只读展示（`frontCountdowns`），repeat-badge + cat-badge（work=蓝/life=绿/study=紫/exercise=橙/diet=琥珀/sleep=青，亮暗双主题） |
...
| 工作台便签 | `workbench/WorkbenchNotes.vue` | 类型切换（普通/时光轴 tabs `nt-type-normal`/`nt-type-timeline`，表单 overlay 内 radio `nt-form-type-normal`/`nt-form-type-timeline`，切换保留 content+entries 数据）+ 搜索（`nt-search-input`，大小写不敏感标题+内容）+ 分类筛选 chips（`nt-cat-all`/`nt-cat-uncategorized`/`nt-cat-<id>`）+ 分类管理弹窗（`nt-cat-manager` 入口，改名/上移下移/删除、名称唯一、删除后该分类便签归未分类）+ 时光轴卡片（竖排时间轴 `nt-timeline-card`，快速追加 `nt-entry-add`，条目行内编辑/删除 `nt-entry-edit-<id>`/`nt-entry-del-<id>`）；datetime 校验 `^\d{4}-\d{2}-\d{2} \d{2}:\d{2}$` + 范围检查（月 1-12/日 1-31/时 0-23/分 0-59，纯正则拒绝不了 '2026-13-99 25:61'）；条目排序走 `noteCore.sortTimelineEntries`，筛选走 `filterNotes` |
...
- **WorkbenchNotes.vue testid 约定**：类型 tabs `nt-type-normal`/`nt-type-timeline`、搜索 `nt-search-input`、分类筛选 chips `nt-cat-all`/`nt-cat-uncategorized`/`nt-cat-<id>`、分类管理入口 `nt-cat-manager`（弹窗行 `nt-catmgr-*`）、时光轴快速追加 `nt-entry-add`、条目行内编辑/删除 `nt-entry-edit-<id>`/`nt-entry-del-<id>`、空态 `note-empty`（普通）/`note-timeline-empty`（时光轴）；其余表单 `note-*` / `nt-form-*` 前缀
