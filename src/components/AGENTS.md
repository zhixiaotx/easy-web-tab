...
└── business/              # 销售记账（摆摊进销存）8 SFC：BusinessView 页面的 7 功能面板 + 共享分类管理弹框
    ...
    ├── BusinessInventory.vue   # 库存管理：低库存预警清单（停售商品排除）+ 库存卡片网格（桌面 5 列/≤640px 单列，bizinv-card-<id> 四项：商品/单位/进货合计/带出合计，聚合走 businessCore.calcPurchaseTotals/calcBroughtOutTotals）+ 阈值可配置
