...
└── business/              # 销售记账（摆摊进销存）8 SFC：BusinessView 页面的 7 功能面板 + 共享分类管理弹框
    ...
    ├── BusinessStats.vue       # 统计报表：近 7/14/30 天营业额/成本/利润分组柱状图 SVG 按收摊记录口径（仅收摊日有柱；每日 3 槽位异色、非零渲染、负值向下、顶部金额标签、全部/营业额/利润模式切换 bizstats-mode-*；坐标走 businessCore.businessTrendBars；分类/商品排行已移至 BusinessHome）
