...
//
// 覆盖范围：
//   a) 工作台菜单 9 项可达性：wb-menu-<key> 按钮 + 逐项点击 → 面板 testid 渲染
...
//
// 菜单 9 项契约（workbenchMenuCore.MENU_DEFAULT_LABELS，逐字一致）：
//   home=主页 todos=工作待办 notes=个人便签 countdowns=定时提醒 pomodoro=番茄钟
...
//     倒计时 cd-add-button / 番茄钟 pm-remaining / 习惯 hb-total-count /
...
// 菜单 9 项契约（键序 + 默认标签逐字来自 workbenchMenuCore.MENU_DEFAULT_LABELS）
...
test('1. 打开工作台：9 个菜单项可见，顺序与默认标签逐字正确', async () => {
  await page.goto('http://localhost:16718/workbench', { waitUntil: 'load' })

  // 菜单项（排除折叠按钮与底部的全局搜索入口）恰好 9 个
  ...
  expect(order, `菜单顺序 ${JSON.stringify(order)} 应为 9 项契约序`).toEqual(MENU_KEYS)
...
test('2. 逐项点击 9 个菜单 → 对应面板渲染', async () => {
...
test('7. Ctrl+Alt+1..9 菜单跳转（焦点不在输入框）：8=健康 / 9=记账', async () => {
  ...
  await expect(page.locator('[data-testid="ld-stat-income"]')).toBeVisible()
  await expect(page.locator('[data-testid="wb-menu-ledger"]')).toHaveClass(/active/)
})
