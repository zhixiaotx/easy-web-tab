...
├── workbenchTodos.ts     # 工作台待办 CRUD + 筛选/搜索/排序 + 分类注册表（全自定义分类 customCategories/tabCategories/allCategories + addCategory/updateCategory/deleteCategory/moveCategory/toggleTabCategory 返回 {ok, reason}，重命名同步存量 categoryId、被引用禁删，存量 work/life/study 一次性迁移未分类，偏好存 localStorage） (246 lines, IndexedDB store 'todos')
