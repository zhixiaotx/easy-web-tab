...
├── test-business-core.ts    # businessCore.ts 纯函数测试（18 断言 T1-T18：种子/归一化/内置支出补回/商品分类空数组尊重/分类 CRUD（add/rename/toggle/move/delete）/soldCount/calcDailyRevenue/calcInventory/calcBusinessStats（含负毛利率）/lowStockProducts/排行/趋势/折线坐标；npm run test:business）
...
| 销售记账纯逻辑测试 | `test-business-core.ts` | 18 断言 T1-T18（种子分类/归一化幂等/内置支出补回/商品分类空数组尊重/双分类 CRUD 结果（empty/duplicate/not-found/in-use/builtin/boundary）/soldCount 负值钳 0/calcDailyRevenue 缺失商品计 0/calcInventory/calcBusinessStats（亏损毛利率为负）/lowStockProducts 阈值+升序/商品与分类排行/calcBusinessTrend 窗口补零/businessTrendScale 全零态）；`node --experimental-strip-types` 直跑；`npm run test:business` |
