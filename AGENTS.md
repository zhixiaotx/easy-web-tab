...
- `src/components/AGENTS.md` — the 24 root SFCs + 20 workbench SFCs + 18 student SFCs + 8 business SFCs, sizes, component-level anti-patterns
...
│   │   ├── student/              # 18 SFC 学生工作台面板（首页/计划/习惯/作业/奖励/阅读/复习/错题/课表/考试/番茄/勋章/日记/教育经历 + 家长协同 StudentParent）+ StudentOnboarding/StudentPanelPlaceholder/StudentParentPinDialog
...
| 学生工作台 | `src/views/StudentView.vue` + `src/components/student/` + `src/stores/student*.ts` + `src/composables/student*Core.ts` | 独立页面 `/student`（学习/家长协同）；左菜单由 studentMenuCore 驱动；15 个 studentXxx store（命名 `useStudentXxxStore`）+ 15 个 student*Core 纯函数（全部薄委托 + useIdb 持久化）；面板见 `src/components/AGENTS.md`（18 SFC：首页/计划/习惯/作业/奖励/阅读/复习/错题/课表/考试/番茄/勋章/日记/教育经历 + 家长协同 StudentParent + StudentOnboarding/StudentPanelPlaceholder/StudentParentPinDialog）；家长协同（StudentParent.vue）=6-tab 布局（统计/每日任务/孩子报告/手动加分/发放勋章/配置奖励）+ PIN 锁态（StudentParentPinDialog，4-8 位、5 次错锁 5 分钟），session 守卫在 StudentView；学段 K/P/J 由 studentSettings/studentStageCore（StudentOnboarding 强制） |
...
| Element Plus 表格/分页 | `src/main.ts`（全局注册）+ 各面板 | Element Plus **全量导入**（非按需）+ zh-cn locale；应用于教育经历（StudentEducation）、健康四面板记录弹框、记账查看弹框、学生/业务多处 el-table + el-pagination（10 条/页）；无需逐组件 import |
...
- **Element Plus**: **全量导入**（非按需）在 `src/main.ts` `app.use(ElementPlus, { locale: zhCn })` — 各面板直接用 `<el-table>`/`<el-pagination>` 等，**无需逐组件 import**；用于教育经历（StudentEducation）、健康四面板记录弹框、记账查看弹框、学生/业务多处
...
node scripts/qa-student-ui.mjs  # Playwright UI QA: 学生工作台 UI（后台/复用 vite dev 16718-16726；同端口域，禁与其他 QA 并行）；另有 qa-student-onescreen.mjs / qa-student-reading-grid.mjs
