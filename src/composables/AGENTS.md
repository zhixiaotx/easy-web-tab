...
28 composable files for reusable business logic. One (`presetIcons.ts`) is auto-generated at build time.
...
- `healthCore.ts` / `ledgerCore.ts` / `noteCore.ts` / `diaryCore.ts` are pure-logic engines — **禁止 import vue/pinia**（`node --experimental-strip-types` 测试运行器无法执行）；组件/面板只调用它们，禁止重算公式
