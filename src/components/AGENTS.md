...
└── workbench/            # 个人工作台 18 SFC: 13 面板 + PanelPager 共享分页条 + 健康管理 tabs 容器 + 定时提醒只读区块 + WeatherCard/CalendarAnchorCard 主页内嵌卡 (data persisted to IndexedDB via `useIdb.ts`)
    ...
    ├── WorkbenchCountdown.vue   # 倒计时面板（分类筛选 tabs + ⚙️分类管理弹框 + 规则/分类表单 + 徽标 + 自适应分页 usePanelPaging（rowHeight 158）+ PanelPager（筛选/排序/分类改名/删除/新增 goto(1)），data-testid 前缀 cd-）
    ├── WorkbenchPassword.vue    # 密码面板（主密码三态 + 新增/编辑弹窗 + 书签关联下拉 + 名称搜索 + 自适应分页 usePanelPaging（rowHeight 77）+ PanelPager（搜索变化 goto(1)，锁态列表未渲染时分页惰性），复用 usePasswordsStore / useCrypto.ts）
    ...
    ├── WorkbenchExercise.vue    # 运动面板（周目标 times/minutes/calories + 记录 + 达标率 + 自适应分页 usePanelPaging（rowHeight 118）+ PanelPager（展开/增删改 goto(1)，折叠未挂载时分页惰性），data-testid 前缀 ex-）
    ├── WorkbenchDiet.vue        # 饮食面板（每日热量目标 + 四餐次记录 + 自适应分页 usePanelPaging（rowHeight 118）+ PanelPager（展开/增删改 goto(1)，折叠未挂载时分页惰性），前缀 dt-）
    ├── WorkbenchSleep.vue       # 睡眠面板（每日时长目标 + 入睡/起床时间自动算时长 + 质量星标 + 自适应分页 usePanelPaging（rowHeight 118）+ PanelPager（展开/增删改 goto(1)，折叠未挂载时分页惰性），前缀 sl-）
    ...
    ├── WorkbenchLedger.vue      # 记账面板（月份切换 + 六指标统计含存款累计 + 近 6 月收支趋势柱状图（ld-trend-*，内联 SVG，坐标全走 trendChartScale）+ 支出分类占比环形图（ld-donut-*，R=90 周长不变量）+ 分类占比条 + 行式记录列表可折叠 + 分组管理 + 自适应分页 usePanelPaging（rowHeight 49）+ PanelPager（月份切换/本月/新增/编辑/删除/分组管理 goto(1)，折叠未挂载时分页惰性），前缀 ld-）
